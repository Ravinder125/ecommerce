...
import { IOrder, Order } from "../order/order.model.js";
...
export const Dashboard = asyncHandler(async (req: Request, res: Response) => {
  ...
  const promises = [
    ...
    Order.find({})
      .limit(4)
      .select(["orderItem", "discount", "totalPrice", "orderStatus"]),
  ];
...
export const getPieCharts = asyncHandler(
  async (req: Request, res: Response) => {
    ...
    const discount = orders.reduce(
      (acc: number, order: any) => acc + (order.discount | 0),
      0,
    );

    const productionCost = orders.reduce(
      (acc: number, order: any) => acc + (order.shippingCharge | 0),
      0,
    );

    const burnt = orders.reduce(
      (acc: number, order: any) => acc + (order.taxPrice | 0),
      0,
    );
    ...
    return res
      .status(200)
      .json(new ApiResponse(200, charts, "Pie charts successfully fetched"));
  },
);
...
export const getBarCharts = asyncHandler(
  async (req: Request, res: Response) => {
    ...
    const [lastSixMonthProducts, lastSixMonthUsers, twelveMonthOrders] =
    ...
    const orderCounts = getBarChartData({
      length: 12,
      today,
...
export const getLineCharts = asyncHandler(
  async (req: Request, res: Response) => {
    ...
    const promises = [
      Product.find(getMonthBaseQuery(sixMonthAgo, today)).select("createdAt"),
      User.find(getMonthBaseQuery(sixMonthAgo, today)).select("createdAt"),
      Order.find(getMonthBaseQuery(twelveMonthAgo, today)).select([
        "createdAt",
        "totalPrice",
        "discount",
      ]),
